...
static constexpr eosio::name token_active_perm{"active"_n};
#define XTOKEN_ISSUE(bank, to, quantity, memo) \
    {	xdaotoken::token::issue_action act{ bank, { {_self, token_active_perm} } };\
            act.send( to, quantity, memo );}

#define XTOKEN_RANSFER(bank, to, quantity, memo) \
    {	xdaotoken::token::transfer_action act{ bank, { {_self, token_active_perm} } };\
            act.send( _self, to, quantity , memo );}

#define XTOKEN_OPENACCT(bank, owner, symbol, rampayer) \
    {	xdaotoken::token::open_action act{ bank, { {_self, token_active_perm} } };\
            act.send( owner, symbol, rampayer);}

#define XTOKEN_CREATE_TOKEN(bank, creator, supply, fee_ratio, gas_ratio, fullname) \
    {	xdaotoken::token::create_action act{ bank, { {_self, token_active_perm} } };\
...
namespace xdaotoken
{
     ...
     */
    class [[eosio::contract("xdaotoken111")]] token : public contract
    {
    public:
