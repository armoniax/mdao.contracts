...
void mdaomeeting::init( const name& admin, const extended_asset& quantity ){
    require_auth(_self);
    CHECKC( is_account( admin ), err::RECORD_NOT_FOUND, "admin account not exists" );

    _gstate.admin = admin;
    _gstate.quantity = quantity;
}
...
void mdaomeeting::_create_renew_dao(const name& from, const name& dao_code, const string& group_id, const asset& quantity, const uint64_t& month){
    ...
    auto dao_itr = dao_info( dao_code.value );
    ...
    CHECKC( token_contract == _gstate.quantity.contract,err::PARAM_ERROR,"token contract must be " + _gstate.quantity.contract.to_string())

    asset need_quantity = _gstate.quantity.quantity * month;
    ...
    db::set( _meeting_tbl, itr, _self, [&]( auto& p, bool is_new ){
        ...
        if ( is_new ){
            ...
            p.group_id = group_id;
            p.creator = from;
            p.expired_at = current_time_point();
        }
