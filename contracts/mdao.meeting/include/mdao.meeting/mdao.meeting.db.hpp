...
MEEING_TBL_NAME("global") global_t {
    ...
    bool                    running      = true;
    uint16_t                split_id;
    extended_asset          fee = extended_asset(1000000000,extended_symbol(AMAX_SYMBOL,"amax.token"_n));
    name                    receiver;
    uint64_t                reserved0;
...
MEEING_TBL meeting_t{
    ...
    typedef eosio::multi_index< "meetings"_n, meeting_t ,
    indexed_by<"bycreator"_n, const_mem_fun<meeting_t, uint64_t, &meeting_t::by_creator>>
    > tbl_t;
};
