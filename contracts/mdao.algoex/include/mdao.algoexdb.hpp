...
static constexpr name MDAO_BANK = "mdao.token"_n;
...
namespace db
{
    #define ALGOEX_TBL [[eosio::table, eosio::contract("mdao.algoex")]]
    #define ALGOEX_TBL_NAME(name) [[eosio::table(name), eosio::contract("mdao.algoex")]]
    
    struct ALGOEX_TBL_NAME("global") algoex_global_t
    {
        AppInfo_t app_info;
        ...
        EOSLIB_SERIALIZE(algoex_global_t, (app_info)(exchg_status)(admins)(quote_symbols)(limited_symbols)
            (token_crt_fee)(exchg_fee_ratio))
    };

    typedef eosio::singleton<"global"_n, algoex_global_t> algoex_global_singleton;

    struct ALGOEX_TBL market_t
    {
