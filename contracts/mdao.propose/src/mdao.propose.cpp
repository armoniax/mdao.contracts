...
ACTION mdaoproposal::cancel(const name& owner, const uint64_t& proposal_id)
{
    ...
    CHECKC( ((proposal.created_at + (governance->voting_period * seconds_per_hour)) >= current_time_point()), 
                                ...
                                const string& title)
{
    ...
    bool is_not_expired = (proposal.created_at + (governance->voting_period * seconds_per_hour)) >= current_time_point();
    if ( is_not_expired ) {
